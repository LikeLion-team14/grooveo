...
public class SoundPostReplyController {
    ...
    public String create(Model model, @PathVariable("id") Long id,
        ...
        SoundPostReply soundPostReply = this.soundPostReplyService.create(soundPostComment, replyForm.getContent(), member);
        model.addAttribute("fileInfo", fileInfo);
        model.addAttribute("commentPaging", commentPaging);
        return String.format("redirect:/library/soundDetail/%s?commentPage=%s&so=%s#reply-%s",
                soundPostReply.getSoundPostComment().getFileInfo().getId(), commentPage, so, soundPostReply.getSoundPostComment().getId());
    }
    ...
    public String delete(@PathVariable("id") Long id) {
        ...
        SoundPostComment soundPostComment = this.soundPostCommentService.getComment(soundPostReply.getSoundPostComment().getId());
